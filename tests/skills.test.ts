...
describe("Muse Skills Registry & Catalog Integrity (TDD)", () => {
  ...
  test("skills.json preserves requested priority ordering (#1 updatedocs through #21 humanize)", () => {
  ...
  test("skills.json contains all 21 total skills categorized across 5 divisions", () => {
    const { skills } = JSON.parse(fs.readFileSync(SKILLS_JSON_PATH, "utf8"));
    expect(skills.length).toBe(21);
