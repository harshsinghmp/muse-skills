...
const matches = [...ledger.matchAll(entryPattern)];
for (const m of matches) {
  ...
  const artRefs = evidence.matchAll(
    /`(\.agents\/artifacts\/[^`]+|\.agents\/archive\/[^`]+|\.agents\/context\/[^`]+)`/g,
  );
...
for (const s of auditSkills) {
  ...
  try {
    ...
    /* skill may not have audit reference */
  }
}
