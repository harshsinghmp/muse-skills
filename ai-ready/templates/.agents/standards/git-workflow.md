...
- **Clean Package Syntax & Versioning**: In git/skills package syntax (`<owner>/<repo>#<ref>`), anything following `#` is a git reference. Never append raw commit hashes or arbitrary branch references (`#<ref>`); keep repository links and skill installation commands clean (`skills add <owner>/<repo>`). If a reference is strictly required anywhere, ensure it never breaks linking or downstream resolution. For package managers (`npm`, `bun`, etc.), use `@latest` when specified as a parameter; otherwise keep commands clean without redundant arguments to fetch latest automatically.
