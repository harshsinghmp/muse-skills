...
| **10** | **AI Agent Context Alignment** | If commands or core constraints changed, was `AGENTS.md` / AI context updated with minimal token overhead? | Keep agent operational rules aligned with actual repo capabilities. |
| **11** | **Memory / State Hygiene** | Was project memory / state updated only for durable facts without dumping session noise? | Prevent transient debugging thoughts from becoming permanent state. |
