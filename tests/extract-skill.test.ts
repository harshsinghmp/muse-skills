...
describe("Skill Extraction Helper (extract-skill)", () => {
  ...
  test("Gate 1: Recurrence Gate enforces >= 4 occurrences", () => {
    // Under threshold
    const failRes = checkRecurrenceGate(undefined, 3, false);
    expect(failRes.ok).toBe(false);
    expect(failRes.message).toContain("Pattern recurrence count (3) < 4");

    // Meets threshold
    const passRes = checkRecurrenceGate(undefined, 4, false);
