#!/usr/bin/env bun
/**
 * 🛡️ secret-scan.ts - Sovereign Secret Scanning CLI for Muse Agency OS
 *
 ...
 *   bun scripts/secret-scan.ts . --verbose
 *   bun scripts/secret-scan.ts . --verify
 *   bun run secret-scan [path]
 */
...
async function main() {
  ...
  try {
    ...
    console.error("Install with: brew install trufflehog or your OS package manager");
    process.exit(1);
  }
